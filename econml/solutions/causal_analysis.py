...
from econml.dml import LinearDML, SparseLinearDML, ForestDML, NonParamDML
from econml.inference import BootstrapInference
from econml.utilities import add_intercept, inverse_onehot
import numpy as np
import pandas as pd
...
from ..cate_interpreter import SingleTreePolicyInterpreter, SingleTreeCateInterpreter
import warnings
...
def _get_default_shared_insights_output():
    ...
    return {
        ...
        _CausalInsightsConstants.Version: '0.0',
...
class CausalAnalysis:
    ...
    def fit(self, X, y, warm_start=False):
        ...
        if warm_start and self.nuisance_models != self.nuisance_models_:
            ...
            if self.classification:
                ...
                y = OneHotEncoder(drop='first', sparse=False).fit_transform(
                    y.reshape(-1, 1))
            else:
    ...
    def _summarize(self, *, summary, get_inference, props, n, expand_arr):
        ...
        def coalesce(attr):
            ...
            # concatenate along treatment dimension, then flatten
            return np.concatenate([ensure_proper_dims(attr(get_inference(res)))
    ...
    def _pandas_summary(self, get_inference, props, n,
        ...
        def make_dataframe(props):
            ...
            index = pd.MultiIndex.from_tuples([(i, res.feature_name, f"{lvl}v{res.feature_baseline}"
                                                if res.feature_baseline is not None
                                                else lvl)
                                               for i in range(n)
                                               for res in self._results
                                               for lvl in res.feature_levels],
                                              names=["sample", "feature", "feature_value"])
    ...
    def _dict_summary(self, get_inference, props, n, kind, expand_arr=False):
        def make_dict(props):
    ...
    def global_causal_effect(self, alpha=0.05):
        """
        Gets the global causal effect for each feature as a pandas DataFrame
        ...
        global_effects : pandas Dataframe
            ...
            Rows : For each feautre that is numerical, we have an entry with index ['{feature_name}', 'num'], where
                'num' is literally the string 'num' and feature_name is the input feature name.
                For each feature that is categorical, we have an entry with index ['{feautre_name}', '{cat}v{base}']
    ...
    def _global_causal_effect_dict(self, alpha=0.05):
        ...
        """
        return self._dict_summary(lambda res: res.global_inference, props=self._point_props(alpha), n=1, kind='global')
    ...
    def _cohort_causal_effect_dict(self, Xtest):
        """
        Gets the cohort causal effects for each feature as dictionary.

        Only for serialization purposes to upload to AzureML
        """
        return self._dict_summary(self._cohort_effect_inference(Xtest),
                                  props=self._summary_props(alpha), n=1,
                                  expand_arr=True, kind='cohort')
    ...
    def local_causal_effect(self, Xtest, alpha=0.05):
        ...
        global_effect : pandas Dataframe
            ...
            Rows: For each feature that is numeric, we have an entry with index
                ...
                ['{sampleid', '{feautre_name}', '{cat}v{base}']
                where cat is the category value and base is the category used as baseline.
                If all features are numerical then the feature_value index is dropped in the dataframe, but not
                in the serialized dict.
        """
    ...
    def _local_causal_effect_dict(self, Xtest, alpha=0.05):
        ...
        """
        return self._dict_summary(self._local_effect_inference(Xtest),
                                  props=self._point_props(alpha), kind='local', n=Xtest.shape[0])
    ...
    def whatif(self, X, Xnew, feature_index, y):
        """
        Gets counterfactual predictions when feature_index is changed to Xnew from its observational counterpart
        ...
        # TODO: this works for NormalInferenceResults, but not EmpiricalInferenceResults
        #       (because the empirical samples also need to move)
        #       Ideally, the econml API would support translating inference results by an offset
        inf.pred += y
